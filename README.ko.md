...
이 명령들을 직접 칠 일은 없다. 영상에 대해 에이전트에게 물으면, 에이전트가
`analysis-video`를 집어 들고, 읽을 수 있는 형태로 받아서, **화면에 있던 것**을 근거로
답한다 — 그 위로 흘러간 말만이 아니라.

![세 갈래 — 당신, AI 에이전트, analysis-video. 요약을 요청하면 에이전트가 analyze 를 실행하고, 도구는 split·transcribe·화면 변화 검출을 거쳐 context.md 와 next 객체(do=read, 이미지 6장, 2,652 토큰)를 돌려준다. 에이전트가 읽고 판단하고 쓰는 단계만 모델을 필요로 하며, 그 결과를 review 로 저장하면 next 가 done 이 된다.](docs/media/loop-ko.svg)

슬라이드 위의 설명은 *이것*·*여기*·*저 항* 같은 지시어로 가득하다. 자막은 그 말을 남기고
*가리킨 대상*을 버린다. 이 도구가 되돌려 놓는 것이 그 절반이다.

| | 자막만 있을 때 | `analysis-video` 를 쓸 때 |
|---|---|---|
에이전트가 받는 것 | "…보시다시피 **여기서** 정규화된 값이 나옵니다" | `## 63.13-87.73s` · `[빈 칠판]` `[판서가 끝난 칠판]` · **왼쪽과 똑같은 대사**
답할 수 있는 것 | 말한 내용까지. **"여기"가 무엇인지 모른다.** | "여기"가 가리킨 수식과, 그것이 써지는 데 걸린 24초

**에이전트는 다음에 뭘 할지 스스로 계산하지 않는다.** 모든 명령이 같은 `next` 객체로 끝나고,
`next`는 *이미지가 도착하기 전에* 가격표를 먼저 준다:

```json
{"do": "read",
 "read": ["/abs/lecture.mp4.analysis/runs/full/context.md"],
 "command": "analysis-video review /abs/lecture.mp4 --run full --write -",
 "why": "이 분석 단위의 리뷰가 아직 없습니다",
 "remaining": ["full"],
 "cost": {"images": 6, "image_tokens": 2652,
          "rule": "긴 변이 1568px를 넘으면 1568px까지 줄인 뒤 (가로*세로)/750"}}
```

그래서 컨텍스트 창이 빠듯한 모델은 오래 떠 있던 화면만 골라 열고 나머지를 건너뛸 수 있다 —
`context.md`가 각 화면이 떠 있던 시간을 적어 두기 때문이다. 다 열고 나서 초과를 알게 되는
대신이다.

### 이렇게 물으면 된다

| 당신이 하는 말 | 에이전트가 하는 일 | 손에 남는 것 |
|---|---|---|
"`lecture.mp4` 요약해줘 — 슬라이드에 있던 것도 포함해서" | `analyze` → `context.md` 읽기 → 정리 | 63초 지점의 칠판을 인용하는 요약. 자막 요약이 아님
"`tutorial.mp4`에 나온 코드 전부 뽑아줘" | 오래 떠 있던 화면만 골라서 엶 | 코드 조각들 + 각각이 나온 시각
"`standup.mp4`에서 뭐가 결정됐어?" | 같은 루프 한 바퀴 | 결정 사항 + 그때 화면에 떠 있던 것
"이 강의 폴더 통째로 검색되게 만들어줘" | 파일마다 `analyze` | 영상별 grep 가능한 마크다운, 이미지 링크 유지
"2~5분, 15~20분 구간만" | `--range 120-300 --range 900-1200` | 독립된 분석 단위 두 개. 절대 병합되지 않음

### 분석은 `context.md`가 생겼을 때 끝나지 않는다

에이전트가 쓴 분석문은 `reviews/<단위>.md`에 보관되고, 그때 읽은 `context.md`의 sha256이 함께
찍힌다. 다음 주에 다시 물으면 에이전트는 이미지를 전부 다시 여는 대신 그 글을 재사용한다.
임계를 바꿔 재검출하면 그 분석문은 스스로를 **stale**로 표시한다 — 옛 분석이 현재 사실인 척
조용히 통과하지 않고, 루프가 다시 읽으라고 요구한다.

| 리뷰 상태 | 뜻 |
|---|---|
`missing` | 쓰인 적 없음
`unreadable` | 머리말이 없거나 깨짐
`stale` | 읽었던 `context.md`가 그 뒤 바뀜
`current` | 아직 일치함
...
![순서대로 도는 다섯 단계: split 은 비디오 리소스를 쓰고 자막 트랙을 분리하고, transcribe 는 자막을 우선하며 없으면 Whisper 로 넘어가고, frames 는 이미지 연산으로 화면 변화를 검출하고, context.md 는 화면과 시간과 대사를 담고, review 는 에이전트가 읽고 쓴 것을 담는다. 앞의 넷은 모델 없는 이미지 연산과 파일 입출력이고, review 만 에이전트의 몫이다.](docs/media/pipeline-ko.svg)
...
## 맞는 화면을 남겼다는 걸 어떻게 아나

프레임 선택은 모델이 아니라 픽셀 연산이다 — 하지만 그건 질문을 옮길 뿐이다. **검출기는 자기
결과가 옳은지 스스로 판정할 수 없다.** 선택 설치 GUI는 사람이 그것을 판정하고, 그 판정을
숫자로 바꾸라고 있다.

![analysis-video GUI 의 창 일곱 개가 데모 클립에 대해 모두 열려 있는 화면: Player, 추출된 프레임 6장을 시각·라벨과 함께 보여 주는 Gallery, Frame sync, 현재 대사가 굵게 표시된 Dialogue sync, 검출 결과를 사람이 찍은 정답 플래그와 대조해 채점하는 Compare report, 원본·자막·언어와 열린 창 목록을 관리하는 허브, 그리고 세 검출 신호를 기준선과 함께 그리는 Timeline.](docs/media/gui-workbench.png)

*창 일곱 개가 각각 독립된 OS 창이라 원하는 모니터에 아무렇게나 배치할 수 있다. 오른쪽 허브는
자기 캔버스가 없다 — 원본과 분석 단위, 열린 창 목록을 관리할 뿐이다. 이 그림을 다시 만들려면
`uv run python examples/make_gui_screenshot.py --window workbench`.*

**"여기선 반드시 뽑혔어야 한다"를 찍고, 검출기를 그것으로 채점한다.** 영상을 보다가 프레임이
나왔어야 할 순간에 <kbd>F</kbd>를 누르면 그 순간이 **정답 플래그**로 `user_flags.json`에
기록된다. Compare report는 지정한 허용오차 안에서 그 플래그들과 검출기가 실제로 남긴 프레임을
대조한다:

| | 세는 것 |
|---|---|
**재현율 recall** | *당신이* 찍은 플래그 중 검출기가 찾아낸 비율 — **놓친 것**
**정밀도 precision** | 검출 중 당신의 플래그 근처에 떨어진 비율 — **쓸데없이 뽑은 것**

플래그를 하나도 안 찍었으면 둘 다 비어 있다 — 정답지 없이 매긴 점수는 뒤에 아무것도 없는
숫자다. 허용오차 기본값은 **2초**이고, 행을 클릭하면 재생이 **그 행의 정답 플래그 시각**으로
이동해 숫자에서 언제나 그 화면으로 되짚을 수 있다. `Export compare.json`으로 대조 결과를 파일로
뽑으면 임계값 변경을 인상이 아니라 diff로 이야기할 수 있다. "`--cut-area-threshold`를 올려라"가
감이 아니라 측정이 되는 지점이 여기다.

**창들은 CLI를 따라온다.** 분석 디렉터리 — `metadata.json`, `transcript.json`, 출력 폴더
자체 — 를 감시하다가 바뀌면 다시 그린다. 터미널에서 임계값을 바꿔 `analysis-video analyze`를
다시 돌리면 타임라인·갤러리·리포트가 알아서 갱신된다. 다시 열 것이 없고, 폴링 루프도 없다.
`frames`는 단위 폴더를 지우고 다시 만들기 때문에, 재실행이 도는 동안 그 창들은 잠시 비었다가
끝나는 순간 새 결과로 채워진다. 재시작이 필요한 경우는 하나다 — 분석 단위가 아직 없는
디렉터리에서 GUI를 먼저 열었다면, 그 첫 `frames` 실행은 자동으로 잡히지 않는다.

분석의 본체 — `state.json` · `transcript.json` · `metadata.json` · `frames.json` — 는 임시
경로에 쓰고 이름을 한 번 바꿔 갈아 끼운다. 그래서 타임아웃으로 죽은 실행이 직전 산출물을 잘라
놓지 않는다. **이건 GUI 기능이 아니라 코어의 성질이라**, 창을 한 번도 열지 않는 사람에게도 그대로
적용된다. 다만 전부는 아니다 — 검출 캐시와 단위 목록, 그리고 에이전트가 읽는 `context.md`는 아직
그냥 쓴다. 다시 돌리면 만들어지는 파생물이라 우선순위를 뒤로 뒀다. 그래서 GUI도 원자성에 기대지
않는다: 읽다가 쓰이는 중인 파일을 만나면 그 판을 비워 두고 다음 감시 이벤트에서 다시 읽는다.

이 도구를 쓰는 데 이것들이 필요하지는 않다. GUI는 버전이 따로 도는 별도 패키지라 CLI만 쓰는
사람은 Qt를 내려받지 않는다 — [설치](#저장소에서-바로-설치) 참조.
