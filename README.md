...
**A real one is in this repository.**
[`docs/media/demo-pipeline.mp4.full.review.md`](docs/media/demo-pipeline.mp4.full.review.md)
is what an agent wrote after running the loop on the 41-second demo clip: the header is
generated by the tool, the body is the agent's. Read it for one line in particular — the
clip's narration names two detection signals, the slide on screen names three, and the
review catches that an agent working from the transcript alone would have gotten the
answer wrong. That is the case for frames, made on the tool's own output rather than
asserted. Reproduce it with `analysis-video analyze docs/media/demo-pipeline.mp4`.
